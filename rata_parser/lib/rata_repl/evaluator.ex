defmodule RataRepl.Evaluator do
  ...
  defp resolve_module_function("Maps", function_name) do
    function_atom = String.to_atom(function_name)
    if function_exported?(Maps, function_atom, 0) do
      {:ok, {Maps, function_atom}}
    elsif function_exported?(Maps, function_atom, 1) do
      {:ok, {Maps, function_atom}}
    elsif function_exported?(Maps, function_atom, 2) do
      {:ok, {Maps, function_atom}}
    elsif function_exported?(Maps, function_atom, 3) do
      {:ok, {Maps, function_atom}}
    else
      {:error, "undefined function: Maps.#{function_name}"}
    end
  end
  defp resolve_module_function("List", function_name) do
    function_atom = String.to_atom(function_name)
    if function_exported?(List, function_atom, 0) do
      {:ok, {List, function_atom}}
    elsif function_exported?(List, function_atom, 1) do
      {:ok, {List, function_atom}}
    elsif function_exported?(List, function_atom, 2) do
      {:ok, {List, function_atom}}
    elsif function_exported?(List, function_atom, 3) do
      {:ok, {List, function_atom}}
    else
      {:error, "undefined function: List.#{function_name}"}
    end
  end
  defp resolve_module_function("Table", function_name) do
    function_atom = String.to_atom(function_name)
    if function_exported?(Table, function_atom, 0) do
      {:ok, {Table, function_atom}}
    elsif function_exported?(Table, function_atom, 1) do
      {:ok, {Table, function_atom}}
    elsif function_exported?(Table, function_atom, 2) do
      {:ok, {Table, function_atom}}
    elsif function_exported?(Table, function_atom, 3) do
      {:ok, {Table, function_atom}}
    else
      {:error, "undefined function: Table.#{function_name}"}
    end
  end
  defp resolve_module_function("Set", function_name) do
    function_atom = String.to_atom(function_name)
    if function_exported?(Set, function_atom, 0) do
      {:ok, {Set, function_atom}}
    elsif function_exported?(Set, function_atom, 1) do
      {:ok, {Set, function_atom}}
    elsif function_exported?(Set, function_atom, 2) do
      {:ok, {Set, function_atom}}
    else
